...
(function ($) {
    $(document).ready(function ($) {
        ...
        var listYesno = $('.kseo-yesno-control');
        if (listYesno.length)
        {
            $.each(listYesno, function () {
                var selection = $(this);
                $('.yesno .button', selection).on('click', function (event) {
                    event.preventDefault();
                    var yesno = $(this).closest('.yesno');
                    if (yesno.hasClass('selected'))
                        return;
                    $(yesno).find('input').prop("checked", true);
                    var container = yesno.closest('.kseo-yesno-control');
        ...
        var formlabel = 0;
        var tgm_media_frame;
         $('body').on('click', '.kseo-controls .kseo-upload-button', function () {
            formlabel = $(this).closest('.kseo-controls');
            if (tgm_media_frame) {
        ...
        $('body').on('keyup paste', '.kseo-controls .kads-controls-count', function (e) {
            var elSetting = $(this).closest('.kseo-controls');
        ...
        $('body').on('click', '.kseo-features-option-set .features-remove', function (e) {
         ...
         $('body').on('click', '.kseo-post-meta .kads-seo-tab-control', function (e) {
           var action = $(this).attr('action');
           var group = $(this).closest('.kads-seo-tab-groups');
           $('.kads-seo-tab-control',group).removeClass('active');
           $('.kads-seo-tab-content',group).removeClass('active');
           
           $(this).addClass('active');
           
           $('.kads-seo-tab-' + action).addClass('active');
            e.preventDefault();
        });
